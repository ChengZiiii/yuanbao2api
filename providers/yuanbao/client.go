// Package yuanbao is the yuanbao provider implementation of the
// providers.Provider contract. It wraps the upstream Yuanbao (腾讯元宝)
// web chat API and exposes the same request/response surface as any
// other adapter registered in provider.Registry.
//
// The package is a self-contained re-implementation of the legacy
// top-level yuanbao/ package; the old import path is removed in
// section 11 of the multi-provider-routing change.
package yuanbao
...
// It accepts both raw JSON payloads and "data: <json>" SSE framing.
// Empty lines, non-data lines, and [DONE] markers all return (nil, nil).
func ParseStreamLine(line string) (*YuanbaoResponseChunk, error) {
	...
	var payload []byte
	if bytes.HasPrefix(trimmed, []byte("data: ")) {
		payload = trimmed[6:] // "data: " is pure ASCII, byte slice safe
	} else {
		payload = trimmed[5:] // "data:" is pure ASCII, byte slice safe
	}
